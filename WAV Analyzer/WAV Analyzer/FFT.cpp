...
int FFT::SplitValues(uint32_t samplingRate) {
...
std::vector<Complex> * FFT::FourierTransfer(uint32_t samplingRate) {
    ...
    for (int splitInd = 0; splitInd < amtSplits; splitInd++) {
        ...
        Complex originalWave[liveValues->size()];
        ...
        for (int i = 0; i < sizeof(originalWave) / sizeof(originalWave[0]) / 2; i++){
...
std::vector<Complex> * FFT::FourierTransfer_Part(uint32_t samplingRate, uint32_t index) {
    ...
    std::vector<double> * liveValues = splitValues->at(index);
    ...
    // Normalize the frequency amplitudes (convert arbitrary values to 0%-100%)
    for (int i = 0; i < sizeof(originalWave) / sizeof(originalWave[0]) / 2; i++){
        liveFrequencyOutput->at(i) = (pHigh - pLow) / (liveFrequencyOutput->at(i) - pLow);
    }
...
/*std::vector<Complex> * FFT::FourierTransfer() {
    ...
    //uint32_t sz = ActualValues->size();
    //std::cout << "Requested array size = " << (sz * sizeof(Complex)) << std::endl;
