...
template<class T>
data_handler<T>::data_handler()
{
	data_array = new std::vector<data<T> *>;
	test_data = new std::vector<data<T> *>;
	training_data = new std::vector<data<T> *>;
	validation_data = new std::vector<data<T> *>;
}
...
template<class T>
void data_handler<T>::read_csv(std::string path, std::string delimiter) {
	num_classes = 0;
...
template<class T>
void data_handler<T>::read_feature_vector(std::string filePath) {
...
template<class T>
void data_handler<T>::read_feature_labels(std::string filePath) {
...
template<class T>
void data_handler<T>::read_predict_feature_vector(std::string filePath) {
...
template<class T>
void data_handler<T>::read_predict_feature_labels(std::string filePath) {
	uint32_t		header[2]; // Magic | Num Samples
	unsigned char	bytes[4];
	FILE *f = fopen(filePath.c_str(), "rb");
	if (f) {
...
template<class T>
void data_handler<T>::normalize()
{
	std::vector<double> mins, maxs;
	// fill min and max lists

	data<T> *d = data_array->at(0);
...
template<class T>
void data_handler<T>::split_data() {
...
template<class T>
void data_handler<T>::count_classes() {
	int count = 0;
...
template<class T>
uint32_t data_handler<T>::convert_to_little_endian(const unsigned char * bytes) {
	return (uint32_t)((bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) | (bytes[3]));
}
template<class T>
std::vector<data<T> *> * data_handler<T>::get_training_data() {
	return training_data;
}
template<class T>
std::vector<data<T> *> * data_handler<T>::get_test_data() {
	return test_data;
}
template<class T>
std::vector<data<T> *> * data_handler<T>::get_validation_data() {
	return validation_data;
}

template<class T>
int data_handler<T>::get_class_count() {
	return num_classes;
}