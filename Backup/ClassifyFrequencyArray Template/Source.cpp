...
template<class T>
int mainKNN(data_handler<T> *dh) {
	knn<T> * knearest = new knn<T>();
...
template<class T>
int mainKMEANS(data_handler<T> * dh) {
...
template<class T>
int mainNetwork(data_handler<T> *dh)
{
...
template<class T>
int mainKNN_Predict(data_handler<T> *dh) {
	knn<T> * knearest = new knn<T>();
	knearest->set_training_Data(dh->get_training_data());
	knearest->set_test_Data(dh->get_test_data());
	knearest->set_validation_Data(dh->get_validation_data());
...
int main(int argc, char ** argv) {
	if (argc == 1) {
		/// Generic Learning
		data_handler<float>* dh = new data_handler<float>();
		//dh->read_feature_vector("C:/Users/stein/Desktop/Research Project 2020/Neural Networks/MNIST/Training Data/Training Data/train-images.idx3-ubyte");
		//dh->read_feature_vector("../Training Data/train-images.idx3-ubyte");
		...
		return mainKNN_Predict<float>(dh); // Notice that this is unconventional use of exit codes to pass data
	}
